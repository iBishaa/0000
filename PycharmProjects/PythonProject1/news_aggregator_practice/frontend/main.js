...
if (typeof Chart === 'undefined') {
    console.error('Chart.js не загружен! Проверьте CDN.');
}
let allArticles = [];
...
async function loadData() {
  ...
  // В функции loadData()
const fetchNews = await fetch(`${API_BASE}/fetch/${STUDENT_ID}`, {
  method: "POST",
  headers: {
  ...
  mode: "cors",
  credentials: "include"  // Для передачи кук, если используются
});

if (!fetchNews.ok) throw new Error(`HTTP error! status: ${fetchNews.status}`);
}
